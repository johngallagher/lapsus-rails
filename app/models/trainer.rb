  ...
  def self.train_entry(entry, mode)
    Container.for_user(entry.user).each do |container|
      if container.contains_project_for_entry?(entry)
        ...
        entry.project = Project.where(preset: true).first
      elsif mode == :last_active
        none = Project.where(preset: true).first
        previous_entry = Entry.where('finished_at <= ? AND project_id != ?', entry.started_at, none.id).order('finished_at DESC').limit(1).first
        if previous_entry
